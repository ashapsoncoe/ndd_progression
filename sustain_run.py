...
input_data_z_table_path =  'GLM_gaussian_max_deg_2_age_TIV_centre_grouped_regions_onlyage2_sep_sex\\preclindementia152_23_regions_z_scores.csv'
all_data_file = 'UKBW_EBM_78867r674582_250923v2.csv'  
dataset_name = 'sustain_output_152cases_23regions'
...
if __name__ == '__main__':
    ...
    input_data_z_df = pd.read_csv(f'{working_dir}\\{input_data_z_table_path}', index_col=0)
    all_data_df = pd.read_csv(f'{working_dir}\\{all_data_file}')
    ...
    output_folder = working_dir + f'//{dataset_name}_{num_events_per_biomarker}events_{N_startpoints}startpoints_{N_S_max}maxclusters_{N_iterations_MCMC}iterations_{input_data_z_arr.shape[1]}biomarkers'
    ...
    with open(f'{output_folder}\\cross_validation_results_{N_folds}_fold.pkl', 'wb') as fp:
